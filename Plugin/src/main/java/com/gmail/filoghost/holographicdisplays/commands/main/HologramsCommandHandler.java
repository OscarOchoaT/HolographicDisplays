...
public class HologramsCommandHandler implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(Colors.PRIMARY + "Server is running custom version of holographic displays by NulledCode#5189");
        ...
        for (HologramSubCommand subCommand : subCommands) {
            if (subCommand.isValidTrigger(args[0])) {
                ...
                if (!subCommand.hasPermission(sender)) {
                    sender.sendMessage(Colors.ERROR + "No permissions.");
                    return true;
                }
